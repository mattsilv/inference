...
async function main() {
  ...
  try {
    ...
    models.forEach(model => {
      if (!model.pricing) {
        warnings.push(`⚠️ Model ${model.displayName} (${model.systemName}) has no pricing data`);
        return;
      }
      
      const vendorName = model.vendor?.name?.toLowerCase() || '';
      const vendorKey = Object.keys(VENDOR_PATTERNS).find(key => 
      ...
      if (vendorPatterns) {
        vendorSpecificValidation(model, vendorPatterns, vendorKey, pricingIssues);
      }
    });
    ...
    if (pricingIssues.length > 0) {
      const logFile = path.join(__dirname, '..', 'pricing-validation-errors.log');
...
function validatePrice(model, priceField, issues, warnings) {
  ...
  if (price > 0 && price < VALIDATION_THRESHOLDS.suspiciouslyLow) {
    issues.push(`🚨 CRITICAL: ${model.displayName} ${priceField} price of $${price} is suspiciously low. Should be in dollars per MILLION tokens.`);
  }
  
  // Check for extremely high values
  const highThreshold = priceField === 'inputText' 
    ? VALIDATION_THRESHOLDS.suspiciouslyHighInput 
    : VALIDATION_THRESHOLDS.suspiciouslyHighOutput;
    
  if (price > highThreshold) {
    issues.push(`🚨 CRITICAL: ${model.displayName} ${priceField} price of $${price} is unusually high. Verify this is correct per MILLION tokens.`);
  }
}
...
function vendorSpecificValidation(model, vendorPatterns, vendorName, issues) {
  ...
  if (inputPrice && vendorPatterns.inputRanges) {
    ...
    if (inputPrice < min || inputPrice > max) {
      issues.push(`⚠️ ${model.displayName} input price of $${inputPrice} is outside the typical range for ${vendorName} (${min}-${max})`);
    }
  }
  ...
  if (outputPrice && vendorPatterns.outputRanges) {
    ...
    if (outputPrice < min || outputPrice > max) {
      issues.push(`⚠️ ${model.displayName} output price of $${outputPrice} is outside the typical range for ${vendorName} (${min}-${max})`);
    }
  }
  ...
  if (vendorName === 'google') {
    ...
    if (inputPrice > 10 && inputPrice < 50) {
      issues.push(`🚨 Google pricing issue detected: ${model.displayName} input price of $${inputPrice} is likely wrong. Google typically prices around $0.0375 per million CHARACTERS for input, not tokens.`);
    }
    
    if (outputPrice > 100 && outputPrice < 200) {
      issues.push(`🚨 Google pricing issue detected: ${model.displayName} output price of $${outputPrice} is likely wrong. Google typically prices around $0.15 per million CHARACTERS for output, not tokens.`);
    }
  }
}
