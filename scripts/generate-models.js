...
function validateModel(model, vendorFile) {
  ...
  // Required fields
  const requiredFields = ['id', 'systemName', 'displayName', 'categoryId', 'parametersB', 'vendorId', 'host'];
  
  for (const field of requiredFields) {
    if (model[field] === undefined) {
      errors.push(`Model ${model.id || 'unknown'} in ${vendorFile} is missing required field: ${field}`);
    }
  }
  
  // Pricing validation
  if (!model.pricing) {
    errors.push(`Model ${model.id} in ${vendorFile} is missing pricing information`);
  } else {
    ...
    for (const field of requiredPricingFields) {
      if (pricing[field] === undefined) {
        errors.push(`Model ${model.id} in ${vendorFile} pricing is missing required field: ${field}`);
      }
    }
    
    // Validate modelId matches parent model id
    if (pricing.modelId !== model.id) {
      errors.push(`Model ${model.id} in ${vendorFile} has mismatched modelId (${pricing.modelId}) in pricing`);
    }
    
    // Pricing values should be numbers and not negative
    for (const field of ['inputText', 'outputText', 'finetuningInput', 'finetuningOutput', 'trainingCost']) {
    ...
    try {
      ...
      for (const model of vendorModels) {
        ...
        if (modelIds.has(model.id)) {
          validationErrors.push(`Duplicate model ID ${model.id} found in ${vendorFile}`);
        } else {
        ...
        if (model.pricing && pricingIds.has(model.pricing.id)) {
          validationErrors.push(`Duplicate pricing ID ${model.pricing.id} found in ${vendorFile}`);
        ...
        if (!categories.some(c => c.id === model.categoryId)) {
          validationErrors.push(`Model ${model.id} in ${vendorFile} references non-existent category ID: ${model.categoryId}`);
        }
        
        // Verify vendor exists
        if (!vendors.some(v => v.id === model.vendorId)) {
          validationErrors.push(`Model ${model.id} in ${vendorFile} references non-existent vendor ID: ${model.vendorId}`);
        }
